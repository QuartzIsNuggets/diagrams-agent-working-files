...
- **PDF-direct — ruled out.** A vector PDF with the editor's baked label paths (MathJax's TeX
  font) is only *close* to the paper's typography (≈ Computer Modern, not the document's exact
  typesetting); for a serious math paper "close" isn't good enough, and TikZ already serves
  that need better. Non-paper uses are covered by SVG. So PDF-direct earns no place.
...
- The **label language is LaTeX in both directions**: the LaTeX string you type is rendered by
  MathJax on screen and passed straight through to TikZ for the paper — one label format, no
  translation. (Confirms the MVP's "type LaTeX, MathJax renders it" choice.)
